...
class CustomersController < ApplicationController
    ...
    def update_lifecycle
        ...
        # LOG.debug(fname){"params=#{params.inspect}"}
        @object_plm = Customer.find(params[:id])
        ctrl_promote(@object_plm) if commit_promote?
        ctrl_demote(@object_plm) if commit_demote?
        ctrl_revise(@object_plm) if commit_revise?
    end
    ...
    def index_
        @object_plms = Customer.find_paginate(user: current_user, filter_types: params[:filter_page], page: params[:page], query: params[:query], sort: params[:sort], nb_items: get_nb_items(params[:nb_items]))
    end

    def customer_params
        params.require(:customer).permit(:id, :owner_id, :typesobject_id, :statusobject_id, :next_status_id, :previous_status_id,:ident, :revision, :designation, :description, :date, :owner, :group_id, :projowner_id, :domain, :type_values)
    end
