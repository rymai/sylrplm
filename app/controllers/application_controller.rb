...
class ApplicationController < ActionController::Base
  ...
  #
  def check_database_consistency(*_args)
    fname = "#{self.class.name}.#{__method__}"
  ...
  def check_access_data(*_args)
    ...
    #
    #
    ret = if @_params[:controller] == 'main' && @_params[:action] == 'index'
            true
          else
            true # false
          end
  ...
  def get_session_theme(session)
    ...
    if session[:user_id]
      if user = User.find(session[:user_id])
        ret = user.theme unless user.theme.nil?
      end
    else
      ret = session[:theme] if session[:theme]
    end
  ...
  def current_user
    ...
    ret = @current_user
    if ret.nil?
      ret = User.find(session[:user_id]) if session[:user_id]
    end
    ...
    if !obj.modelname.nil? && obj.respond_to?(:typesobject) && !obj.typesobject.nil?
      ...
      unless File.exist?("#{Rails.root}/public#{ret}")
        ...
        unless File.exist?("#{Rails.root}/public#{ret}")
          ...
          ret = '' unless File.exist?("#{Rails.root}/public#{ret}")
        end
      end
    else
      # no type
  ...
  def icone_plmtype(plmtype)
    ...
    ret = '' unless File.exist?("#{config.root}/public#{ret}")
    ret
  end
