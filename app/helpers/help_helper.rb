...
module HelpHelper
  def h_help(key)
    ...
    if msg.nil?
      ...
      title = nil
      txt = if hlp.nil?
              t(:help_no_help, key: key)
            else
              h_help_transform(hlp)
            end
  ...
  def h_help_elem(elem, key)
    ...
    if el.nil?
      elem.each_element('msg') do |element|
  ...
  def h_help_suitable(root_help, key)
    ...
    unless msg.nil?
      unless msg[:el].nil?
        unless msg[:el].previous_element.nil?
  ...
  def h_help_level(elem, with_anchor = true)
    ...
    unless elem.nil?
      ...
      msg += h_help_ul(elem)
      elem.elements.each('msg') do |element|
  ...
  def h_help_ul(elem)
    msg = ''
    elem.elements.each('ul') do |elem_ul|
      msg += "<ul class='help_ul'>\n"
      unless elem_ul.attributes['title'].nil?
      ...
      msg += h_help_transform(elem_ul.text)
      elem_ul.elements.each('li') do |elem_li|
        msg += "<li class='help_ul'>\n"
        if !elem_li.attributes['title'].nil?
          ...
          msg += "</li>\n"
          msg += h_help_transform(elem_li.text) unless elem_li.text.nil?
          msg += h_help_ul(elem_li)
        else
          msg += h_help_transform(elem_li.text) unless elem_li.text.nil?
          msg += h_help_ul(elem_li)
          msg += "</li>\n"
  ...
  def h_help_transform(hlp)
    ...
    txt.each_byte do |c|
      # decode+=c.to_s+" "
      special = false if c != 10 && c != 13 && c != 32 && c != '\t'
    end
    txt = h_help_replace(txt) unless special
    txt + "\n"
  end
  ...
  def h_help_replace(txt)
    ...
    txt.gsub!('.' + 10.chr, '.<br/>')
    txt.gsub!('\t', '')
    txt.gsub!(/#bloc=(.+?)=bloc#/) { |bloc| PlmServices.exec_bloc(bloc, Regexp.last_match(1).to_s) }
    #
    txt.gsub! /#hlp=(.+?)=hlp#/,
  ...
  # help on a chapter
  def help_chapter_rails2(chapter, cls = nil)
    ret = ''
  ...
  def help_chapter(chapter, cls = nil)
    if cls.nil?
      link_to   t(chapter), onclick: "return helpWindow('#{chapter}');"
    else
      link_to   t(chapter), onclick: "return helpWindow('#{chapter}');", class: cls
    end
  end

  def help_window_rails2(cls = nil)
    ret = ''
  ...
  def help_window(cls = nil)
    if cls.nil?
      ...
      link_to t(:help_window), onclick: "return helpWindow('help_#{params[:controller]}_#{params[:action]}');", class: cls
    end
  end
  ...
  def show_help(key, with_img = true)
    txt = t(key.to_s)
    if with_img
      bloc = "<img class='help' id='#{key}'  src='/images/help.png' onclick=\"return helpPopup('#{key}');\" >"
    else
      bloc = "<a href='/help?help=#{key}' onclick=\"return helpPopup('#{key}');\">#{txt}</a>"
    end
    bloc.html_safe
  end
end
