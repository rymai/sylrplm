...
class Typesobject < ActiveRecord::Base
  ...
  #
  MODELS_PLM = %w[customer document part project].freeze
  NO_TYPE = '-NO_TYPE-'
  ...
  def others
    ...
    res = ::Typesobject.all.where("forobject='#{forobject}'").to_a
    LOG.debug(fname) { "Typesobject:self=#{inspect} others=#{res.inspect}" }
    #
    if res.is_a?(Array)
      ret = res
    else
  ...
  def self.get_types(s_object, with_generic = false)
    ...
    Typesobject.all.order(:name).to_a.each do |type|
      ...
      next unless type.forobject.to_s == s_object.to_s
  ...
  def get_fields_values_type_only
    ...
    ret.each do |key, _value|
      ret.delete(key) if key.start_with?('_type_only_')
    end
  end
  ...
  def get_fields_values_(key = nil)
    ...
    if respond_to? :fields
      unless fields.blank?
        ...
        ret = decod_json(fields, key, name)
      end
    end
    # LOG.debug(fname) {"key=#{key} ret=#{ret}"}
    unless ret.nil?
      ret = ret[key] unless key.nil?
    end
    LOG.debug(fname) { "key=#{key} ret=#{ret}" }
    ret
  end
