class Link < ActiveRecord::Base
	...
	def initialize(*args)
		...
		LOG.debug(fname) {"initialize self fin=#{self.inspect}"}
	end
	...
	def link_uniqueness_obsolete
		...
		unless ::Link.find(:first, :conditions => cond).nil?
			self.errors.add(:base,'Link already exists!')
		ret=false
		end
	...
	def validity
		...
		#LOG.info(fname) {"child.typesobject=#{child.typesobject} relation.child_typesobject=#{relation.child_typesobject}"}
			...
			unless valid
				self.errors.add(:base,"Link is not valid:<br/>#{msg}")
			end
		rescue Exception => e
			valid = false
	...
	def cardinality_count
		...
		unless nb_used >= relation.cardin_use_min && (relation.cardin_use_max == -1 || nb_used <= relation.cardin_use_max)
			self.errors.add(:base,'Cardinality is not valid!')
		end
	end
	...
	def self.is_child_of(father_plmtype, father, child_plmtype, child)
		...
		nb=all.where("father_plmtype='#{father_plmtype}' and child_plmtype='#{child_plmtype}' and father_id =#{father} and child_id=#{child}").to_a.count
		ret=nb>0
	end
