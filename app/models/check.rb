...
class Check < ActiveRecord::Base
  ...
  def validity
    ...
    begin
      ...
      when CHECK_STATUS_FREE
        ...
        valid = false
        errors.add(:status, I18n.translate('activerecord.errors.messages.status_not_valid'))
      end
    # LOG.info(fname) {"status=#{status} in_user=#{in_user} in_group=#{in_group} in_reason=#{in_reason} in_date=#{in_date} valid=#{valid}"}
    rescue Exception => e
      valid = false
      errors.add(:base, I18n.translate('activerecord.errors.messages.exception_validity', model: 'check', exception: e))
    end
    valid
  end
  ...
  def user_obsolete=(user)
    ...
    self.out_group_id = user.group_id
    self.projowner_id = user.project_id
    LOG.debug(fname) { "check:user= self=#{inspect}" }
  end
