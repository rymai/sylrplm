...
class Relation < ActiveRecord::Base
  ...
  def validate
    errors.add :base, I18n.t('valid_relation_cardin_occur_max') if !cardin_occur_max.nil? && cardin_occur_max != -1 && cardin_occur_max < cardin_occur_min
    errors.add :base, I18n.t('valid_relation_cardin_use_max') if !cardin_use_max.nil? && cardin_use_max != -1 && cardin_use_max < cardin_use_min
  end
    ...
    unless ret.nil?
      unless relation_type_name.blank?
        ...
        ret.each do |rel|
          ...
          ret1 << rel if rel.typesobject.name == relation_type_name
        end
        ret = ret1
      end
      unless relation_paste_way.blank?
        ...
        ret.each do |rel|
          ...
          ret1 << rel if rel.paste_way == relation_paste_way
        end
        ret = ret1
      end
    end
  ...
  def self.by_values_and_name(father_plmtype, child_plmtype, _father_type, _child_type, name)
    fname = "Relations.#{__method__}:"
  ...
  def stop_tree?
    PlmServices.get_property(:TREE_RELATION_STOP).include?(name)
  end
end
