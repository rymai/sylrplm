...
include Zip
	...
	def update_attributes_repos(params, user)
		...
		begin
			...
			if(uploaded_file)
				...
				LOG.debug(fname){"update_attributes_repos: uploaded_file: self=#{self.inspect}"}
			else
			...
			self.errors.add(:base,"Error update datafile attributes : #{e}")
			ret=false
	...
	def find_col_for(strcol)
		fname= "#{self.class.name}.#{__method__}"
		Sequence.find_col_for(self.modelname,strcol)
	end
