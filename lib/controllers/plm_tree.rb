...
def build_tree(obj, view_id, variant_mdlid = nil, level_max = 9999)
  ...
  variant = if variant_mdlid.nil?
              nil
            else
              PlmServices.get_object_by_mdlid(variant_mdlid)
            end
  var_effectivities = if variant.nil?
                        []
                      else
                        variant.variant_effectivities
                      end
...
def associate_users(node, father)
  ...
  node << usersnode unless usersnode.empty?
  if father.respond_to?(:childs)
    ...
    node << organode unless organode.empty?
  end
...
def follow_tree(root, node, father, relations, var_effectivities, level, level_max)
  ...
  if father.modelname == 'project'
    ...
    node << projownersnode unless projownersnode.empty?
  end
...
def add_model(root, node, father, relations, var_effectivities, level, level_max, mdl_child)
  ...
  unless snode.nil?
    ...
    node.remove(snode) if snode.empty?
  end
  # LOG.debug(fname) {"fin de type  #{mdl_child} node.size=#{node.size}"}
end

def add_link(root, node, snode, father, relations, var_effectivities, level, level_max, _mdl_child, link)
  fname = function_name(level, __method__)
  ...
  unless link_effectivities.nil?
    link_effectivities = nil if link_effectivities.empty?
  end
  #
  ...
  if link_to_show == true
    ...
    show_relation = if relations.nil? || relations.count == 0
                      true
                    else
    ...
    if show_relation
      ...
      # taille du noeud
      thenode.label += "(#{thenode.size})" unless thenode.empty?
      if snode.nil?
        node << thenode
...
def add_child(root, father, link, child, level)
  ...
  if child.nil?
    ...
    options = {
      ...
      title: clean_text_for_tree(link.tooltip.to_s),
...
def add_owners(tree, obj)
  ...
  add_owner(tree, obj.projowner) if obj.respond_to? :projowner
  tree
end

def add_owner(tree, father)
  unless father.nil?
    url = { controller: get_controller_from_model_type(father.modelname), action: 'show', id: father.id.to_s }
...
# @return the node after adding other node(s)
#
def follow_father(modelname, node, obj, _relations)
  ...
  links.each do |link|
    begin
      ...
      url = { controller: get_controller_from_model_type(modelname), action: 'show', id: father.id.to_s }
      relation = Relation.find(link.relation_id)
      ctrl_name = "ctrl_#{father.modelname}"
