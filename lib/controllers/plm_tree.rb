...
module Controllers
	module PlmTree
		...
		def build_tree(obj, view_id, variant = nil, level_max = 9999)
			fname="plm_tree:#{obj.controller_name}.#{__method__}"
			lab=I18n.t(:ctrl_object_explorer, :typeobj => I18n.t("ctrl_"+obj.model_name), :ident => obj.label)
		...
		def follow_tree(root, node, father, relations, var_effectivities, level, level_max)
			fname="plm_tree:#{root.controller_name}.#{__method__}"
			...
			#------------------------------------------------------
			usersnode = tree_level("'users_#{father.id}'", I18n.t("label_#{father.model_name}_users"), icone_plmtype("user"), icone_plmtype("user"))
			...
			PlmServices.get_property(:TREE_ORDER).each do |mdl_child|
				if PlmServices.get_property(:TREE_GROUP)
					snode = tree_level("'grp_#{father.id}'", I18n.t("label_#{father.model_name}_#{mdl_child}"), icone_plmtype(mdl_child), icone_plmtype(mdl_child))
				else
					snode = nil
				end
		...
		def tree_users(node, father)
			fname="plm_tree:#{father.controller_name}.#{__method__}"
		...
		def build_tree_up(obj, view_id)
			lab=I18n.t(:ctrl_object_referencer, :typeobj => I18n.t("ctrl_"+obj.model_name), :ident => obj.label)
		...
		def group_tree(thenode, level)
			...
			fname="plm_tree:#{thenode.controller_name}.#{__method__}"
