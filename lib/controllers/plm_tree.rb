...
def follow_tree(node, father)
  ...
  SYLRPLM::TREE_ORDER.each do |mdl_child|
    ...
    links.each do |link|
      ...
      LOG.info (fname){"relation.type.fields=#{relation.type.inspect}"}
      unless relation.type.fields.nil?
        ...
        edit_link_url = url_for(:controller => 'links',
              ...
              :object_id => father.id)
        edit_link_a = "<a href=#{edit_link_url} title=\"#{link_values}\">#{relation.name}</a>"
      else
        edit_link_a = "#{relation.name}"
      end
      ...
      show_a="<a href=\"#{url_for(show_url)}\">#{t(ctrl_name)}:#{child.ident}</a>"
      options={
        :label  => "#{edit_link_a} | #{show_a} | #{remove_link_a}",
        :icon => icone(child),
        :icon_open => icone(child),
        :title => clean_text_for_tree(child.designation),
        #:url => url_for(show_url),
        :open => false
      }
      html_options = {
        :alt => "alt:"+child.ident
...
def build_tree_up(obj)
  tree = Tree.new({:js_name=>"tree_up", :label=>t(:ctrl_object_referencer, :typeobj => t("ctrl_"+obj.model_name)),:open => true})
  SYLRPLM::TREE_ORDER.each do |mdl_child|
...
def follow_father(model_name, node, obj)
  ...
  links.each do |link|
    ...
    options={
      :label => "#{relation.name} | #{t(ctrl_name)}:#{father.ident}",
      :icon => icone(father),
