...
def export_header(columns)
	...
	columns.each do |col|
		if(show? col)
		ret+=col.ident+::SYLRPLM::EXPORT_COLUMN_SEP
		end
	end
	ret+="\n"
	# fin de header
...
def export_explorer (object,method,col,type_table)
	fname="#{self.class.name}.#{__method__}"
...
def export_comma(object,method,col,type_table)
	fname="#{self.class.name}.#{__method__}"
	LOG.info(fname){"object=#{object}, method=#{method}"}
	comma_string(object, method)
end

def export_text(object,method,col,type_table)
	fname= "#{self.class.name}.#{__method__}"
