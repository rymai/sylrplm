...
class PlmServices
  ...
  def self.unzip_stringio(content)
    ...
    ret = nil
    if content.blank?
      raise Exception, 'Content to unzip is null or empty'
    else
  ...
  def self.set_property_cache(prop_name, value)
    fname = "PlmServices.#{__method__}"
    prop_name = prop_name.to_s
    @@plmcache['property'] = {} if @@plmcache['property'].nil?
  ...
  def self.get_object(type, id)
    ...
    unless mdl.nil?
      ...
      begin
        ret = if mdl.to_s == 'Ruote::Sylrplm::Process' || mdl.to_s == 'Process'
                ::RuoteKit.engine.process(id)
              else
                mdl.find(id)
              end
      rescue Exception => e
  ...
  def self.get_properties(atype_ident = nil)
    ...
    array_types.each do |typ|
      ...
      if atype_ident.nil?
        ...
        # just a type
        ret[ident] = typ.get_fields_values if ident == atype_ident
      end
    end
  ...
  def self.get_property(prop_name, atype_ident = nil)
    ...
    if ret.nil?
      ...
      props.each do |key, value|
        ...
        if value.nil? || value[prop_name].nil?
  ...
  def self.ruote_init
    ...
    pg_db_config['host'] = db_config['host']
    ...
    RuoteKit.engine.participant_list.each { |pe| LOG.debug(fname) {" #{pe}"} }
    #
  ...
  def self.file_write(content, repos)
    unless content.empty?
      ...
      begin
        ...
        raise Exception, "Error writing in server file #{repos}:#{e}"
      end
      f.close
    end
  end
