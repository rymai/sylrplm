...
class FiledriverFog < Filedriver
  ...
  def dir_datafile(datafile)
    ...
    vol_dir = datafile.volume.dir_name.tr('_', '-')
    ...
    ret = ret[1, ret.length - 1] if ret.start_with?('.', '-')
    ret = ret.downcase
    ret
  end
  ...
  def create_directory(datafile)
    ...
    if directory_exists?(directory_key)
      directory_key
    else
  ...
  def remove_file(dirkey, filekey)
    ...
    unless dir.nil?
      ...
      dir.destroy if dir.files.empty?
    end
  end
