...
namespace :sylrplm do
  ...
  task :import_domain, [:path, :domain, :site] => [:connect, :environment] do |_t, args|
  ...
  task :export_domain, [:path, :domain] => [:connect, :environment] do |_t, args|
    ...
    unless models.nil?
      ...
      Object.subclasses_of(ActiveRecord::Base).each do |model|
        ...
        begin
          ...
          # model.columns.reject{ |c| c.primary}. each do |col|
          model.columns.each do |col|
            cols << col.name.to_s unless %w[created_at updated_at].include?(col.name)
          end
          ...
          warn "Error during decoding seed file:#{e}"
          seed_file.close
          return
        end
  ...
  def import_seed
    ...
    unless models.nil?
      ...
      begin
        ...
        warn "Error during import seed file:#{e}:stack=\n#{stack}"
        seed_file.close
        return
