...
class RelationsControllerTest < ActionController::TestCase
	...
	test "should create relation" do
		...
		assert_difference('Relation.count') do
			post :create, :relation => {:father_typesobject_id=>1045584123, :child_typesobject_id=>1045584116, :paste_way=>"manual", :cardin_occur_min=>0, :cardin_occur_max=>-1, :cardin_use_min=>0, :cardin_use_max=>-1,
      	:typesobject_id=>502955464, :name=>"relation01", :father_plmtype=>"part",:child_plmtype=>"part"}, :fonct=>{:current=>"create"}
		end
		assert_response :success
