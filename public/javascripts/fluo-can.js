...
var FluoCon = {
	...
	//syl :RGB_WHITE: 'rgb(255, 255, 255)',
	//syl :RGB_HIGHLIGHT: 'rgb(220, 220, 220)',
	RGB_WHITE: 'rgb(230, 230, 230)',
	RGB_HIGHLIGHT: 'rgb(190, 190, 190)',
	//syl : LINE_HEIGHT: 20,
	LINE_HEIGHT: 12,
...
var FluoCanvas = function() {
	...
	function drawText (c, text, bwidth, bheight, symbolFuncName) {
		...
		//c.translate(-(w/2), 17);
		c.translate(-(w/2), 10);
		//var decal=bwidth/2-w/2-4
		//c.translate(-decal-(w/2), 10);
		if (symbolFuncName) {
			c.translate(0, -4);
		...
		c.translate(+(w/2-w0), -10);
		//c.translate(+decal+(w/2-w0), -10);
	...
	function lineInCircle (c, angle, farRadius, closeRadius) {
		...
		c.moveTo(far[0], far[1]);
		c.lineTo(close[0], close[1]);
	}
...
var FluoCan = function() {
	...
	function attributeLineCount (exp) {
		var count = 0;
		for (var attname in exp[1]) {
			var text = ''+attname+': '+fluoToJson(exp[1][attname], false);
			if(attname == 'activity') {
	...
	function carriageReturn (c) {
		if (c.canvas.horizontal == true)
			c.translate(-FluoCon.LINE_HEIGHT, 0);
		else
	...
	function drawAttributes (c, exp, expname, strchild, width, height) {
		...
		while (attname = attnames.shift()) {
			...
			if (attname.match(/^on[-\_]error$/)) {
				...
				var text  = attname+ ": " + v;
				if(attname == 'activity') {
					//alert('drawAttributes:'+attname)
					var tablo = text.split('\n');
					for (var i=0; i<tablo.length; i++) {
						FluoCanvas.drawText(c, tablo[i] , width, height);
						carriageReturn(c);
					}
				} else {
					FluoCanvas.drawText(c, text , width, height);
					carriageReturn(c);
				}
			}
		}
	...
	function newHandler (parentHandler) {
		...
		var result = {};
		for (var k in parentHandler)
			result[k] = parentHandler[k];
		for (var k in parentHandler)
			result['super_'+k] = parentHandler[k];
		return result;
	}

	function getChildren (c, exp) {
		var cs = exp[2];
		if ( ! c.canvas.hideMinor)
			return cs;
		if (exp.majorChildren)
			return exp.majorChildren;
		var r = [];
		for (var i = 0; i < cs.length; i++) {
			var c = cs[i];
			if (MINORS.indexOf(c[0]) < 0)
				r.push(c);
		}
	...
	GenericHandler.render = function (c, exp) {
		...
		//syl FluoCanvas.drawRoundedRect(c, width, height, 8);
		FluoCanvas.drawRoundedRect(c, width, height, 4);
		c.save();
	...
	GenericHandler.getRealHeight = function (c, exp) {
		...
		//syl return 7 + (1 + attributeLineCount(exp)) * FluoCon.LINE_HEIGHT;
		return 2 + (1 + attributeLineCount(exp)) * FluoCon.LINE_HEIGHT;
	};
	...
	GenericWithChildrenHandler.render = function (c, exp) {
		...
		if (c.canvas.horizontal == true)
			c.translate(attHeight/2, 0);
		drawAttributes(c, exp, true, false, attWidth, attHeight);
		c.restore();
		c.save();
		//syl c.translate(width/2 - childrenMax(c, exp, 'getWidth')/2 - 7, 8);
		c.translate(width/2 - childrenMax(c, exp, 'getWidth')/2 - 7, 4);
		for (var i = 0; i < children.length; i++) {
			var child = children[i];
			renderExp(c, child);
			//c.translate(0, 7 + FluoCan.getHeight(c, child));
			c.translate(0, FluoCan.getHeight(c, child));
			if (this.drawArrow && i < children.length -1)
				FluoCanvas.drawArrow(c, 10);
			c.translate(0, 10);
		}
		c.restore();
	};
	...
	VerticalHandler.adjust = function (exp) {
		if (attributeCount(exp, true) > 0)
			exp[2].unshift([ '_atts_', exp[1], [] ]);
	}
	...
	HorizontalHandler.getHeaderHeight = function (c, exp) {
		if (c.canvas.horizontal == true)
			return 23 + attributeMaxWidth(c, exp);
	...
	HorizontalHandler.renderHeader = function (c, exp, distribution) {
		//alert('HorizontalHandler.renderHeader:c='+c+' exp='+exp)
		var hheight = this.getHeaderHeight(c, exp) - 10;
		c.save();
		c.translate(0, 10);
		FluoCanvas.drawQuadraticPath(
		//syl c, distribution[0], hheight, 4);
		c, distribution[0], hheight, 8);
		FluoCanvas.drawQuadraticPath(
		//syl c, distribution[distribution.length-1], hheight, 8);
		c, distribution[distribution.length-1], hheight, 4);
	...
	HorizontalHandler.renderHeaderLabel = function (c, exp) {
		//alert('HorizontalHandler.renderHeaderLabel:c='+c+' exp='+exp)
	...
	HorizontalHandler.renderFooter = function (c, exp, distribution) {
		...
		if (distribution.length == 1) {
			...
			FluoCanvas.drawQuadraticPath(
			//syl c, distribution[0], -10, 8);
			c, distribution[0], -10, 4);
			FluoCanvas.drawQuadraticPath(
			//syl c, distribution[distribution.length-1], -10, 8);
			c, distribution[distribution.length-1], -10, 4);
			for (var i = 1; i < distribution.length - 1; i++) {
	...
	ConcurrenceHandler.renderHeaderSymbol = function (c) {
		//alert('ConcurrenceHandler.renderHeaderSymbol:c='+c)
	...
	IfHandler.adjust = function (exp) {
		...
		for (var i = 0; i < 2 - exp[2].length; i++)
			exp[2].push([ '_',{}, [] ]);
	...
	var MINORS = [
	//syl 'set', 'set-fields', 'unset', 'description'
	'set', 'set-fields', 'unset', 'description', 'define'
	];

	var DEFINERS = [
	//syl 'process-definition', 'workflow-definition', 'define'
	'process-definition', 'workflow-definition'
	];

	function identifyExpressions (exp, expid) {
		if (exp.expid)
			return; // identify only once
		if ( ! expid)
			expid = '0';
		exp.expid = expid;
	...
	function renderFlow (context, flow, options) {
		...
		//syl context.mozTextStyle = "12px Helvetica Neue";
		//syl context.font = "12px Helvetica Neue";
		context.mozTextStyle = "9px Helvetica Neue";
	...
	function isSubprocessName (exp, name) {
		if ((typeof exp) == 'string')
			return false;
		if (DEFINERS.indexOf(exp[0]) > -1 && exp[1]['name'] == name)
			return true;
		for (var i = 0; i < exp[2].length; i++) {
			if (isSubprocessName(exp[2][i], name))
				return true;
		}
		return false;
	}
	...
	function getHeight (c, exp) {
		if ((typeof exp) == 'string')
			return getHandler(c, exp).getHeight(c, exp);
		if (exp.height)
			return exp.height;
		var h = getHandler(c, exp);
		if (h.adjust && ! exp.adjusted) {
			h.adjust(exp);
			exp.adjusted = true;
		}
	...
	function getWidth (c, exp) {
		if ((typeof exp) == 'string')
			return getHandler(c, exp).getWidth(c, exp);
		if (exp.width)
			return exp.width;
		var h = getHandler(c, exp);
		if (h.adjust && ! exp.adjusted) {
	...
	function getHandler (c, exp) {
		...
		if (exp[2].length > 0)
			return GenericWithChildrenHandler;
		if (isSubprocessName(c.canvas.flow, exp[0]))
